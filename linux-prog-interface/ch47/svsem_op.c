...
static void usageError(const char *progName)
{
    ...
    fprintf(stderr, "      %s 12345 0=0n1 1+1, 2-1u 1=0\n", progName);
    exit(EXIT_FAILURE);
}
...
static int parseOps(char *arg, struct sembuf sops[])
{
    ...
    for (numOps = 0, remaining = arg; ; numOps++)
    {
        if (numOps > MAX_SEMOPS)
            cmdLineErr("Too many operations (maxium=%d): \"%s\"\n", MAX_SEMOPS, arg);
        ...
        // Convert the prefix of remaining to long, stores a pointer to unconverted suffix in &sign, using 10 as base
        sops[numOps].sem_num = strtol(remaining, &sign, 10);

        if (*sign == '\0' || strchr("+-=", *sign) == NULL)
            cmdLineErr("Expected '+', '-', '=' in \"%s\"\n", arg);
